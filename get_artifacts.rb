...
res = open("#{host}project/#{user}/#{repo}?circle-token=#{token}")
p res.status[0]
...
latest_build = result.find do |elm|
  elm['branch'] == 'develop' && elm['status'] == 'success'
end

build_num = latest_build['build_num']

puts 'get artifact list'
res = open("#{host}project/#{user}/#{repo}/#{build_num}/artifacts?circle-token=#{token}")
p res.status[0]
exit if res.status[0] != '200'
...
result.each do |elm|
  ...
  open(file_name, 'wb') do |output|
    open("#{url}?circle-token=#{token}") do |data|
